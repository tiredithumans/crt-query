...
///
/// A completed result is exit 0, and that is `main.rs`'s `EXIT_OK` rather than
/// a second constant here: `main.rs` owns the exit-code contract, and a mirror
/// is a value that can drift from the thing it mirrors.
fn on_stdout(f: impl FnOnce(&mut dyn Write) -> io::Result<()>) -> Result<()> {
    on_stdout_with(crate::EXIT_OK, f)
}
...
mod tests {
    ...
    /// The row shape every list-column record shares. The empty case is the
    /// one worth pinning: it must still write a row, or a certificate with no
    /// SANs vanishes from the file while the table shows it.
    #[test]
    fn expand_column_writes_one_row_per_value_and_one_row_for_none() {
        let cells = || vec!["1".to_string(), "joined".to_string(), "z".to_string()];
        let expanded = expand_column(cells(), 1, &["a".to_string(), "b".to_string()]);
        assert_eq!(
            expanded,
            vec![
                vec!["1".to_string(), "a".to_string(), "z".to_string()],
                vec!["1".to_string(), "b".to_string(), "z".to_string()],
            ]
        );
        assert_eq!(expand_column(cells(), 1, &[]), vec![cells()]);
    }
